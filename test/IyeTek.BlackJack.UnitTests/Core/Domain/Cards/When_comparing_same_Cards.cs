...
{
    public class When_comparing_same_Cards: Specification
    {
        ...
        {
            FirstCard = new Card(BlackJackCardType.Eight, SuitType.Spades);
            SecondCard = new Card(BlackJackCardType.Eight, SuitType.Spades);
        }

        public void Then_the_First_and_Second_Card_should_be_the_Same()
        {
            FirstCard.Should().Be(SecondCard);
        }
